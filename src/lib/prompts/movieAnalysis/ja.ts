...
1. **テーマ別の推薦**: 推薦作品を2〜3のテーマに分類します。
   - **テーマ見出し**: 見出し3と引用（\`> ###\`）を使用します。
   - **映画アイテム**:
     - **タイトル**: 映画のタイトルを太字（\`**タイトル**\`）にします。
     - **スコア**: スコアをコードフォーマットで表示します（\`\`\`Score: 0.XXXX\`\`\`）。
     - **説明**: 以下の3点を含む1つの段落。
       1. **概要**: どのような映画か簡潔に説明します。
       2. **スコア**: 具体的なスコア値に言及します。
       3. **関連性**: ユーザーのお気に入りのみに言及し、*なぜ*適しているかを論理的に説明します。
2. **要約セクション**: 最後のセクション。
   - **フォーマット**: 水平線（\`---\`）で区切り、見出し3（\`### まとめ\`）を使用します。
